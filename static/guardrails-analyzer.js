...
function displayResults() {
   ...
   // --- START OF NEW RENDERING LOGIC ---
    ...
    // 2. Actionable Recommendations (Strategic Brief Style)
    const recsHTML = `
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mt-8 relative overflow-hidden fade-in">
            <div class="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-bl-full opacity-60 pointer-events-none"></div>

            <div class="relative z-10">
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 flex items-center gap-3 font-heading">
                            <span class="flex items-center justify-center w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100">
                                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>
                            </span>
                            Strategic Recommendations
                        </h2>
                        <p class="text-slate-500 mt-2 ml-14 max-w-2xl text-sm">
                            Prioritized remediation steps generated by the AI Audit Crew.
                        </p>
                    </div>
                    
                    <button id="toggleRecsBtn" class="px-4 py-2 rounded-lg bg-white border border-slate-200 text-slate-600 text-sm font-medium hover:text-indigo-600 hover:border-indigo-200 transition-all shadow-sm flex items-center gap-2 group">
                        <span id="toggleRecsText">Hide Actions</span>
                        <svg class="w-4 h-4 transition-transform group-hover:-translate-y-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" /></svg>
                    </button>
                </div>

                <div id="recsContent" class="space-y-3 transition-all duration-300">
                    ${analysisResults.recommendations.map((rec, i) => `
                        <div class="group flex items-start gap-4 p-5 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-indigo-200 hover:shadow-md transition-all duration-200 cursor-default relative overflow-hidden">
                            ...
                            <div class="flex-shrink-0 mt-0.5">
                                <div class="w-7 h-7 rounded-lg bg-white border border-slate-200 flex items-center justify-center shadow-sm group-hover:border-indigo-200 group-hover:text-indigo-600 transition-colors">
                                    <span class="text-xs font-bold text-slate-400 group-hover:text-indigo-600">${i + 1}</span>
                                </div>
                            </div>

                            <div class="flex-1">
                                <p class="text-slate-700 text-sm font-medium leading-relaxed group-hover:text-slate-900">
                                    ${escapeHtml(rec)}
                                </p>
                            </div>

                            <div class="opacity-0 group-hover:opacity-100 transition-opacity self-center text-indigo-300 transform translate-x-2 group-hover:translate-x-0 duration-200">
