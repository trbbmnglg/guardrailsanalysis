...
// Version: 3.3 (Flat UI Fix)

(function() {
    'use strict';

    // ... [Keep MODEL_TIERS and MECHANISM_COSTS exactly as they were in v3.2] ...
    // ... [Keep getOptimizationTips and analyzeProfile exactly as they were in v3.2] ...

    // I will only show the updated "renderReport" to save space. 
    // Assume other functions exist.
    ...
    function analyzeProfile(guardrails) {
        let totalBaseLatency = 30; let highestTier = 1; let breakdown = [];
        guardrails.forEach(g => {
            let tier = 1; let mechLabel = "Standard Check"; let mechKey = "regex"; 
            if (g.complexity_tier) { tier = g.complexity_tier; mechLabel = `AI Classified (Tier ${tier})`; } 
                ...
                const data = MECHANISM_COSTS[mechKey]; tier = data.tier; mechLabel = data.label;
            }
            if (tier > highestTier) highestTier = tier;
            let baseCost = 5; if (tier === 2) baseCost = 80; if (tier === 3) baseCost = 800; if (tier === 4) baseCost = 2500;
        ...
        const tierKey = `tier${highestTier}`; const model = MODEL_TIERS[tierKey];
        let finalLatency = 0; breakdown.forEach(item => { const multiplier = item.tier > 1 ? model.latencyFactor : 1; finalLatency += (item.baseCost * multiplier); });
    ...
    function renderReport(guardrails) {
        ...
        initCalculator(data.model.avgCostPer1M);
    }

    function initCalculator(costPerMillion) {
        ...
        const update = () => {
            if (!slider || !volDisplay || !costDisplay) return;
            const reqs = getVolume(slider.value);
            const totalTokens = reqs * AVG_TOKENS_PER_REQ;
    ...
    function getLatencyColor(ms) {
        ...
        if (ms < 800) return "text-blue-600"; // Increased threshold for GPT-5 speeds
